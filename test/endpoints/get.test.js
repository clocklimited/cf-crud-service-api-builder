...
describe('GET endpoint', () => {
  ...
  describe('GET /prefix/:id', () => {
    test('should respond with a 200 when service#read() returns an object', (done) => {
    ...
    test('should respond with a 404 when service.read() doesn’t return anything', (done) => {
      setup((ignore, service) => {
        const app = express()
        createGetEndpoint(service, '/things', app, logger, [], null, hooks)
        request(app)
      ...
      setup((ignore, service) => {
        ...
        request(app)
          ...
          .end((err, res) => {
            if (err) return done(err)
            assert.deepEqual(
              { error: 'Error reading item from "thing" service' },
              res.body
            )
            done()
          })
  ...
  describe('GET /prefix', () => {
    ...
    test('should respond with a 200 and list of items when service.find() returns some objects', (done) => {
      const app = express()
      createGetEndpoint(service, '/things', app, logger, [], null, hooks)
      request(app)
        .get('/things')
    ...
    test('should respond with a 400 and list of items when service.find() errors', (done) => {
      ...
      request(app)
        ...
        .end((err, res) => {
          if (err) return done(err)
          assert.deepEqual(
            { error: 'Error finding items from "thing" service' },
            res.body
          )
          done()
        })
    ...
    test.skip('should successfully paginate requests', (done) => {
      ...
      request(app)
        .get(
          `/things?${qs.stringify({
            pagination: JSON.stringify({ page: 3, pageSize: 7 })
          })}`
        )
        .expect(200)
        .end((err, res) => {
